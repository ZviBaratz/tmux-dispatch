...
### Syntax check
```bash
bash -n dispatch.tmux && bash -n scripts/helpers.sh && bash -n scripts/dispatch.sh && bash -n scripts/preview.sh && bash -n scripts/actions.sh && bash -n scripts/git-preview.sh && bash -n scripts/session-preview.sh && bash -n scripts/session-new-preview.sh
```
...
Nine shell scripts, no build step:
...
- **`scripts/helpers.sh`** — Sourced by all other scripts. Provides `get_tmux_option`, tool detection (`detect_fd`, `detect_bat`, `detect_rg`, `detect_zoxide`, `detect_popup_editor`, `detect_pane_editor`), file history/frecency, bookmarks (`bookmarks_for_pwd` + `all_bookmarks`), and fzf visual options. Handles Debian/Ubuntu renamed binaries (e.g., `fdfind`, `batcat`).
- **`scripts/dispatch.sh`** — Main script with twelve modes via `--mode=files|grep|git|dirs|sessions|session-new|windows|rename|rename-session|scrollback|commands|marks|resume`. Scrollback mode has two views (lines/tokens) toggled by `Ctrl+T`. Uses fzf's `become` action to switch modes mid-session. Handles mode-specific fzf configuration, indicators (bookmarks ★, git status icons), Nerd Font file icons (`_icon_awk_fragments()`), and result actions.
- **`scripts/actions.sh`** — Extracted action handlers called by fzf bindings: file edit, grep edit, delete, rename, bookmark toggle/remove, git stage/unstage, session list/rename/kill, smart-open (dispatches by token type), and preview helpers.
- **`scripts/doctor.sh`** — Health check diagnostic (standalone + commands mode entry).
...
- **`scripts/session-new-preview.sh`** — Preview command for session-new mode (Ctrl+N). Git repos: branch + tracking info + dirty files + recent commits. Non-git dirs: tree/ls cascade.
